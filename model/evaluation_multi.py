...
def run_test(sess, model, batch_gen, data):
    ...
    if IS_LOGGING:
        with open( '../../MY/speech-emotion-recognition/analysis/inference_log/mult.txt', 'w' ) as f:
            f.write( ' '.join( [str(x) for x in list_pred] ) )
        
        with open( '../../MY/speech-emotion-recognition/analysis/inference_log/mult_label.txt', 'w' ) as f:
